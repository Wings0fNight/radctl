...
const UserAccess = () => {
    ...
    const accessCitys = [																				//FIXME: ПЕРЕДЕЛАТЬ НА ФУНКЦИЮ РАЗБОРА ДАННЫХ ПОСЛЕ ИХ ПОЛУЧЕНИЯ
		{ id: 1, group: "Групповые", city: "Москва", layer: "CORE", access: "RO"},
    ...
    return (
        <div className='w-[93%] mx-auto mt-10 mr-20 h-[70%] max-h-[90%] p-6 bg-white border border-gray-300 rounded-lg shadow-md mb-10'>
            ...
            <div className='flex justify-between my-4'>
                <div className='grid grid-cols-2'>
                    ...
                    <input onChange={(e) => setText(e.target.value)} type="text" placeholder="Город (default: ВСЕ)" className="block px-0 w-full text-smbg-transparent border-0 border-b-2 appearance-none text-gray-900 border-gray-600 focus:border-blue-500 focus:outline-none focus:ring-0  peer" />
                </div>
                <div className='grid grid-cols-2'>
                    <label htmlFor="" className='text-right pr-2 font-medium'>Группы: </label>
                    <select onChange={(e) => setSelect(e.target.value)} className="select select-bordered w-full max-w-xs border-gray-600" defaultValue={"All"}>
                        <option>All</option>
            ...
            <div className="relative overflow-x-auto h-[calc(100%-110px)]">
                <table className="w-full text-sm">
                    <tbody className='text-gray-900 text-sm font-medium max-h-[70%]'>
                        {
							select === "All" ?
                            filter.map(accessCity =>
                                <tr className='border-b border-gray-400' key={accessCity.id}>
